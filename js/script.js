...
if (currentGeminiKeyIndex >= GEMINI_API_KEYS.length) currentGeminiKeyIndex = 0;
...
function rotateGeminiKey() {
  ...
  showToast(
    `Gemini API 키 전환됨: ${currentGeminiKeyIndex + 1}번 키 사용`,
    "info"
  );
  return true;
}
...
async function fetchWithRotation(urlBuilder, type = "youtube") {
  const maxRetries =
    type === "youtube" ? YOUTUBE_API_KEYS.length : GEMINI_API_KEYS.length;
  let attempts = 0;
  ...
  while (attempts < maxRetries) {
    try {
      ...
      if (response.status === 403) {
        ...
        console.warn(
          `API Key ${type} index ${
            type === "youtube" ? currentYoutubeKeyIndex : currentGeminiKeyIndex
          } failed (403). Rotating...`
        );
        const rotated =
          type === "youtube" ? rotateYoutubeKey() : rotateGeminiKey();
        if (!rotated) {
          alert(
            "모든 API 키의 할당량이 초과되었습니다. 설정에서 다른 키를 선택하거나 새로운 키를 입력해주세요."
          );
          openSettings();
...
function loadFilters() {
  ...
  document.getElementById("durationFilter").value = duration || "long_shorts";
...
function openSettings() {
  ...
  document.getElementById("inputYoutubeKey1").value = YOUTUBE_API_KEYS[0] || "";
  document.getElementById("inputYoutubeKey2").value = YOUTUBE_API_KEYS[1] || "";
  document.getElementById("inputYoutubeKey3").value = YOUTUBE_API_KEYS[2] || "";

  document.getElementById("inputGeminiKey1").value = GEMINI_API_KEYS[0] || "";
  document.getElementById("inputGeminiKey2").value = GEMINI_API_KEYS[1] || "";
  document.getElementById("inputGeminiKey3").value = GEMINI_API_KEYS[2] || "";

  // Check active keys
  const yRadios = document.getElementsByName("activeYoutubeKey");
  if (yRadios[currentYoutubeKeyIndex])
    yRadios[currentYoutubeKeyIndex].checked = true;

  const gRadios = document.getElementsByName("activeGeminiKey");
  if (gRadios[currentGeminiKeyIndex])
    gRadios[currentGeminiKeyIndex].checked = true;
...
function saveApiKeys() {
  ...
  const selectedYIndex = parseInt(
    document.querySelector('input[name="activeYoutubeKey"]:checked')?.value || 0
  );
  const selectedGIndex = parseInt(
    document.querySelector('input[name="activeGeminiKey"]:checked')?.value || 0
  );

  // Validate index range and existence
  if (selectedYIndex >= YOUTUBE_API_KEYS.length) {
    alert("선택된 YouTube API 키가 비어있습니다. 유효한 키를 선택해주세요.");
    return;
  }
  if (selectedGIndex >= GEMINI_API_KEYS.length) {
    alert("선택된 Gemini API 키가 비어있습니다. 유효한 키를 선택해주세요.");
    return;
  }
...
function resetAllSettings() {
  if (
    confirm(
      "⚠️ 경고: 모든 설정과 데이터(API 키, 즐겨찾기 등)가 완전히 삭제됩니다.\n\n정말 초기화하시겠습니까?"
    )
  ) {
    localStorage.clear();
...
async function loadFavoritesFeed() {
  ...
  favoriteChannels.forEach((ch) => {
    ...
    chip.innerHTML = `
      ...
      ch.title
        ...
        <button onclick="event.stopPropagation(); toggleFavorite(event, '${
          ch.id
        }', '${ch.title}')" class="text-gray-500 hover:text-red-400 ml-1">
  ...
  if (window.DEV_MODE || localStorage.getItem("DEV_MODE") === "true") {
      ...
      return [
        {
          ...
          snippet: {
            ...
            publishedAt: new Date(
              Date.now() - 1000 * 60 * 60 * 24 * (idx + 1)
            ).toISOString(), // 1일씩 차이
            thumbnails: {
              medium: {
                url: `https://picsum.photos/320/180?random=${idx * 10 + 1}`,
              },
            },
          },
        ...
        },
        {
          ...
          snippet: {
            ...
            channelId: ch.id,
            publishedAt: new Date(
              Date.now() - 1000 * 60 * 60 * 12 * (idx + 1)
            ).toISOString(),
            thumbnails: {
              medium: {
                url: `https://picsum.photos/320/180?random=${idx * 10 + 2}`,
              },
            },
          },
          channelColor: ch.color,
    ...
    dummyVideos.forEach((v) => {
      dummyStats[v.id.videoId] = {
        ...
        duration: `PT${Math.floor(Math.random() * 10) + 1}M${Math.floor(
          Math.random() * 60
        )}S`, // 1~10분
      };
    });
    ...
    const filteredVideos = dummyVideos.filter((v) =>
      activeChannelIds.includes(v.snippet.channelId)
    );
    ...
    filteredVideos.forEach((video, index) => {
      ...
      card.innerHTML = `
            ...
            <div class="p-5 flex flex-col flex-grow">
                ...
                <h3 class="text-base font-bold text-white mb-3 line-clamp-2 leading-snug" title="${safeTitle}">${
        video.snippet.title
      }</h3>
                ...
                <div class="grid grid-cols-2 gap-2 mb-4 bg-black/20 rounded-lg p-3 border border-white/5">
                    ...
                    <div class="text-center border-l border-t border-white/10 pt-2 mt-1">
                        ...
                        <div class="font-bold text-blue-400 text-xs">${formatKoreanNumber(
                          viewsPerHour
                        )}/hr</div>
                    </div>
                </div>

                <button onclick="openAnalysisFromFeed('${vidId}', '${safeTitle}', '${
        video.snippet.thumbnails.medium.url
  ...
  try {
    ...
    const activeChannels = favoriteChannels.filter((c) =>
      selectedFeedChannels.includes(c.id)
    );
    ...
    const statsRes = await fetchWithRotation(
      (key) =>
        `https://www.googleapis.com/youtube/v3/videos?part=statistics,contentDetails,snippet&id=${videoIds}&key=${key}`,
      "youtube"
    );
    ...
    const chRes = await fetchWithRotation(
      (key) =>
        `https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${channelIds}&key=${key}`,
      "youtube"
    );
      ...
      card.innerHTML = `
            ...
            <div class="p-5 flex flex-col flex-grow">
                <div class="flex justify-between items-start mb-3">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="text-sm font-bold text-white hover:text-blue-400 cursor-pointer" onclick="window.open('https://www.youtube.com/channel/${
                              video.snippet.channelId
                            }', '_blank')">${video.snippet.channelTitle}</span>
                        </div>
                    </div>
                    <button onclick="toggleFavorite(event, '${
                      video.snippet.channelId
...
async function searchVideos() {
  ...
  try {
    ...
    let searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=20&q=${encodeURIComponent(
      query
...
function renderVideos(videos) {
  ...
  if (videos.length === 0) {
    ...
    videos.forEach((video, index) => {
      ...
      card.style.animationDelay = `${index * 50}ms`;
      const cardBorderColor = isHighPerformer
        ? "rgba(239, 68, 68, 0.5)"
        : "rgba(255, 255, 255, 0.1)";
      card.style.border = `1px solid ${cardBorderColor}`;

      card.innerHTML = `
            <div class="relative group cursor-pointer" onclick="window.open('https://www.youtube.com/watch?v=${
              video.id
            }', '_blank')">
                <img src="${
                  video.snippet.thumbnails.medium.url
        ...
        video.snippet.title
                ...
                <div class="grid grid-cols-2 gap-2 mb-4 bg-black/20 rounded-lg p-3 border border-white/5">
                    ...
                    <div class="text-center border-l border-white/10">
                        <div class="text-xs text-gray-500 mb-1">구독자</div>
                        <div class="font-semibold text-white text-xs">${
                          video.hiddenSubs || video.subCount === 0
                            ? "비공개"
                ...
                <button onclick="openAnalysis('${video.id}', '${safeTitle}', '${
        video.snippet.thumbnails.medium.url
      }', '${safeChannel}', ${ratio}, ${
        video.hiddenSubs
